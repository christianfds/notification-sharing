...
const styles: Record<string, CSSProperties> = {
  ...
  form: { display: 'flex', gap: 8, alignItems: 'end', flexWrap: 'wrap' },
...
export default function CategoryManager() {
  ...
  const load = async () => {
    setLoading(true);
    try { const { data } = await api.get<Category[]>('/categories'); setCategories(data); } catch (cause) { setError(errorMessage(cause)); } finally { setLoading(false); }
  };
  useEffect(() => { void load(); }, []);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setCategories((current) => editing ? current.map((item) => item.id === response.data.id ? response.data : item) : [...current, response.data]);
  ...
  const remove = async (category: Category) => {
    ...
    setBusy(true); setError(null);
    try { await api.delete(`/categories/${category.id}`); setCategories((current) => current.filter((item) => item.id !== category.id)); } catch (cause) { setError(errorMessage(cause)); } finally { setBusy(false); }
  };

  return <section style={styles.panel} aria-labelledby="category-manager-title">
    <div style={styles.heading}><div><h2 id="category-manager-title" style={styles.title}>Categorias</h2><p style={styles.subtitle}>Organize os avisos por assunto.</p></div><button type="button" style={styles.secondary} onClick={() => void load()} disabled={loading || busy}>{loading ? 'Carregando...' : 'Atualizar'}</button></div>
