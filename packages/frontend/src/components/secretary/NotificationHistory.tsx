...
export default function NotificationHistory({ readAtOverrides, onReadStatusChange, refreshKey = 0 }: NotificationHistoryProps) {
  ...
  const markUnread = async (notificationId: string) => {
    await setReadStatus(notificationId, false);
  };

  const markRead = async (notificationId: string) => {
    await setReadStatus(notificationId, true);
  };

  const setReadStatus = async (notificationId: string, read: boolean) => {
    try {
      const { data } = await api.patch<Notification>(`/notifications/${notificationId}/read-status`, { read });
