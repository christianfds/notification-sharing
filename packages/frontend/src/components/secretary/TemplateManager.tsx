...
export default function TemplateManager() {
  ...
  const load = async () => { setLoading(true); try { const { data } = await api.get<Template[]>('/templates'); setTemplates(data); } catch (cause) { setError(errorMessage(cause)); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, []);
  const reset = () => { setEditing(null); setValues({ title: '', body: '' }); };
  const submit = async (event: FormEvent) => {
    ...
    try { const response = editing ? await api.put<Template>(`/templates/${editing.id}`, values) : await api.post<Template>('/templates', values); setTemplates((current) => editing ? current.map((item) => item.id === response.data.id ? response.data : item) : [...current, response.data]); reset(); } catch (cause) { setError(errorMessage(cause)); } finally { setBusy(false); }
  };
  const remove = async (template: Template) => {
    ...
    setBusy(true); setError(null);
    try { await api.delete(`/templates/${template.id}`); setTemplates((current) => current.filter((item) => item.id !== template.id)); } catch (cause) { setError(errorMessage(cause)); } finally { setBusy(false); }
  };
  return <section style={styles.panel} aria-labelledby="template-manager-title">
    <div style={styles.heading}><div><h2 id="template-manager-title" style={styles.title}>Templates</h2><p style={styles.subtitle}>Reutilize textos frequentes ao enviar avisos.</p></div><button type="button" style={styles.secondary} onClick={() => void load()} disabled={loading || busy}>{loading ? 'Carregando...' : 'Atualizar'}</button></div>
